...
let
  relativePath =
    ...
    with builtins;
    strings.removePrefix (toString config.impurity.configRoot) (toString path);

  impurityGroupEnabled =
    group:
    let
...
in
{
  ...
  config._module.args.impurity = rec {
    groupedLink =
      ...
      let
        groups =
          if groupspec == null then
            [ "" ]
          else if (types.listOf types.string).check groupspec then
            groupspec
          else
            assert types.string.check groupspec;
            strings.splitString " " groupspec;
      in
      if config.impurity.enable && lists.any (group: impurityGroupEnabled group) groups then
        createImpurePath path
      else
        path;
